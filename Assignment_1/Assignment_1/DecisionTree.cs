...
namespace Assignment_1
{
    public class DecisionTree
    {
        ...
        public DecisionTree(bool isLeaf, ref List<TrainingData> trainingdata, int value, int depthRemaining, ref List<Features> featuresTaken)
        {
            IsLeaf = isLeaf;
            Children = new List<DecisionTree>();
            ...
            else
            {
                ...
                if (DepthRemaining > 1)
                {
                    for (int i = 0; i < Datas.Count; i++)
                    {
                        ...
                        Children.Add(new DecisionTree(Is_Leaf[i], ref data, LeafValues[i], DepthRemaining - 1, ref featuresTakenHelper));
                    }
                }
                else
                {
